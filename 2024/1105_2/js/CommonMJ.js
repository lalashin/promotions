...
( function(){
    ...
    this.CommonMJ = {
        ...
        FloatBox: function(_itemS, gap) {
            var _win = jQuery(window),
                _wrap = _itemS.parent(),
                wrapT = _wrap.offset().top,
                wrapH = _wrap.outerHeight(),
                wrapB = wrapT + wrapH;
            ...
            _itemS.each(function() {
                var _item = jQuery(this),
                    itemT = _item.offset().top,
                    itemH = _item.outerHeight(),
                    itemB = itemT + itemH;

                _win.scroll(function() {
                    var winT = _win.scrollTop();

                    if (winT <= wrapT - gap) {
                        
                        if (!_item.hasClass('floatbox-top')) {
                            _item.removeClass('floatbox-mid floatbox-bot').addClass('floatbox-top')
                                .css({ position:'absolute', top:0, bottom:'auto' });
                        }

                    } else if (winT >= wrapB - itemH - gap) {
                        
                        if (!_item.hasClass('floatbox-bot')) {
                            _item.removeClass('floatbox-top floatbox-mid').addClass('floatbox-bot')
                                .css({ position:'absolute', top:'auto', bottom:0});
                        }

                    } else if (!_item.hasClass('floatbox-mid')) {
                        _item.removeClass('floatbox-top floatbox-bot').addClass('floatbox-mid')
                            .css({ position:'fixed', top:gap, bottom:'auto' });
                    }
                }).trigger('scroll');
            });
        }
